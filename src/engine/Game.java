...
public class Game {
	...
	public int[][] GetAffected(Ability a)
	{
		int [][] temp=new int[5][5];
		Point currLoc=getCurrentChampion().getLocation();
		int rate=a.getCastRange();
		int i=1;
		while(i<=rate)
		{
			if (currLoc.y+i<5)
			{
				temp[currLoc.x][currLoc.y+i]=1;
				i++;
			}
			else
				break;
	...
	public int[][] GetAffected()
	{
		int [][] temp=new int[5][5];
		Point currLoc=getCurrentChampion().getLocation();
		int rate=getCurrentChampion().getAttackRange();
		int i=1;
		while(i<=rate)
		{
			if (currLoc.y+i<5)
	...
	public boolean isOpponent(Champion Champ)
	{
		Champion main=getCurrentChampion();
		boolean f=false;
		if(firstPlayer.getTeam().contains(main))
			f=true;
		boolean s =false;
		if(secondPlayer.getTeam().contains(Champ))
			s=true;
	    if(s==f)
	    	return true;
		return false;
...
//	}
	...
	public void placeCovers() {
		...
		while (i < 5) {
			...
			int y = ((int) (Math.random() * (BOARDHEIGHT-2)))+1;
...
//	}
	...
	public String move(Direction d) throws NotEnoughResourcesException, UnallowedMovementException {
		...
		return getCurrentChampion().getName()+" Has Moved"+s;
	}
	...
	public String attack(Direction d)
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					if (curhp <= 0)
						board[currx][curry] = null;
					return getCurrentChampion().getName()+" has Damaged a Cover for "+getCurrentChampion().getAttackDamage()+" Points";
					...
					Champion curr = getCurrentChampion();
					 if (hasEffect(target, "Dodge")) {
						...
						if (r <= 50) {
							...
							return target.getName()+" has Dodged "+getCurrentChampion().getName()+"'s Attack!";
						}
					} 
					if (hasEffect(target, "Shield")) {
						for (Effect e : target.getAppliedEffects()) {
							if (e.getName().equals("Shield")) {
								...
								curr.setCurrentActionPoints(curr.getCurrentActionPoints() - 2);
								return target.getName()+" Shielded "+getCurrentChampion().getName()+"'s Attack!";
							}
						}
					}
					...
					targets.add(target);
					cleanup(targets);
					return getCurrentChampion().getName()+" Damaged "+target.getName()+" for "+damage;
				}
			}
		}
	...
	public String castAbility(Ability a)
		...
		if (a.getCastArea() == AreaOfEffect.SELFTARGET) {
			{
				...
				s=getCurrentChampion().getName()+" Buffed Theirself";
			}
			...
			if (a instanceof DamagingAbility || (a instanceof CrowdControlAbility
				...
				s=getCurrentChampion().getName()+" Debuffed ";
				...
				if (firstPlayer.getTeam().contains(getCurrentChampion()))
					...
					team = secondPlayer.getTeam();
				s=getCurrentChampion().getName()+" Buffed ";
			}
			for (Champion c : team) {
			...
			if(a instanceof DamagingAbility || (a instanceof CrowdControlAbility
				&& ((CrowdControlAbility) a).getEffect().getType() == EffectType.DEBUFF))
				s+=targets.size()+" Enemies";
			else if(a instanceof HealingAbility
				|| (a instanceof CrowdControlAbility && (a instanceof CrowdControlAbility
						&& ((CrowdControlAbility) a).getEffect().getType() == EffectType.BUFF)))
				s+=targets.size()-1+" Teammates";
			...
			s+=getCurrentChampion().getName()+" Debuffed "+targets.size()+" Enemies";
		}
	...
	public String castAbility(Ability a, Direction d)
			throws NotEnoughResourcesException, AbilityUseException, CloneNotSupportedException {
		validateCastAbility(a);
		ArrayList<Point> possiblePoints = new ArrayList<Point>();
		int currx = (int) getCurrentChampion().getLocation().getX();
		...
		{
			DamagingAbility temp = (DamagingAbility)a;
			return getCurrentChampion().getName()+" Damaged "+targets.size()+" target/s For "+temp.getDamageAmount();
		}
		else if(a instanceof HealingAbility)
		{
			HealingAbility temp = (HealingAbility)a;
			return getCurrentChampion().getName()+" Healed "+targets.size()+" target/s For "+temp.getHealAmount();
		}
		else
		{
			CrowdControlAbility temp = (CrowdControlAbility)a;
			return getCurrentChampion().getName()+" "+temp.getEffect().getName()+"ed "+targets.size()+" target/s For "+temp.getEffect().getDuration()+" turn/s";
		}
	}
	...
	public String castAbility(Ability a, int x, int y) throws NotEnoughResourcesException, AbilityUseException,
		...
		if (board[x][y] instanceof Cover && a instanceof DamagingAbility)
		{
			...
			s=getCurrentChampion().getName()+" Damaged Cover For "+temp.getDamageAmount();
		}
		else {
			...
			if (hasEffect(c, "Shield")) {
				for (Effect e : c.getAppliedEffects()) {
					if (e instanceof Shield) {
						c.getAppliedEffects().remove(e);
						s=c.getName()+" Shielded "+getCurrentChampion().getName()+"'s Ability";
						break;
					}
				}
			} else
			{
				...
				{
					DamagingAbility temp=(DamagingAbility)a;
					s=getCurrentChampion().getName()+" Damaged "+c.getName()+" For "+temp.getDamageAmount();
				}
				else if(a instanceof HealingAbility)
				{
					HealingAbility temp=(HealingAbility)a;
					s=getCurrentChampion().getName()+" Healed "+c.getName()+" For "+temp.getHealAmount();
				}
				else
				{
					CrowdControlAbility temp=(CrowdControlAbility)a;
					s= getCurrentChampion().getName()+" "+temp.getEffect().getName()+"ed "+c.getName()+" For "+temp.getEffect().getDuration()+" turn/s";
				}
			}
		}
		a.execute(targets);
	...
	public int[][] getSingleAffected(Ability a)
	{
		int[][] temp=new int[5][5];
		int range=a.getCastRange();
		Point currPos=getCurrentChampion().getLocation();
		for(int i=0;i<5;i++)
		{
			for(int j=0;j<5;j++)
			{
				if((Math.abs(i-currPos.x)+Math.abs(j-currPos.y))<=range)
				{
					if(board[i][j]!=null) 
					{
						if(board[i][j] instanceof Champion)
						{
							Champion test=(Champion) board[i][j];
							if(isOpponent(test))
								temp[i][j]=1;
						}
						else
